...
# ===============================================================
# ==== REPLACE the BrainDiffusionPrior class with this code =====
# ===============================================================

class BrainRectifiedFlow(nn.Module):
    """
    Rectified Flow for generating image embeddings from brain activity.
    This class replaces the BrainDiffusionPrior. It uses the same underlying
    network (PriorNetwork) but trains it with a simpler, more direct
    ODE-based objective.
    """
    def __init__(self,
                 ...
                 voxel2clip=None):
        super().__init__()
        self.net = net # The transformer model (PriorNetwork)
    ...
    def sample(self,
        ...
        This is the inference/sampling method.
        """
        device = text_embed.device
        shape = (text_embed.shape[0], self.net.num_tokens, self.image_embed_dim)

        # 1. Start with random noise z_0 at t=0
        z = torch.randn(shape, device=device)
        dt = 1. / num_steps

        # Create conditioning dictionary for the network
        text_cond = dict(text_embed=text_embed)

        # 2. Solve the ODE using the simple Euler method
        for i in range(num_steps):
            time = torch.full((shape[0],), i * dt, device=device)

            # Predict the velocity (the direction to move in)
            velocity = self.net.forward_with_cond_scale(
                z,
                time,
        ...
        return z # The final prediction at t=1

    def forward(self,
                text_embed=None,
                image_embed=None,
                voxel=None,
                *args, **kwargs): # Accept extra args for compatibility
        """
        Calculates the loss for a single training step.
        """
        # Ensure correct inputs are provided
        assert exists(text_embed) ^ exists(voxel), 'either text embedding or voxel must be supplied'
        assert exists(image_embed), 'image embedding must be supplied'

        # If fMRI voxels are provided, use the voxel2clip model to get the conditioning embedding
        if exists(voxel):
            assert exists(self.voxel2clip), 'voxel2clip must be trained if you wish to pass in voxels'
            # Note: The original BrainNetwork forward returns (backbone, clip_proj, blurry_recon)
            # We use the 'backbone' output as the conditioning embedding.
            backbone_embedding, _, _ = self.voxel2clip(voxel)
            text_embed = backbone_embedding

        batch, device = image_embed.shape[0], image_embed.device

        # --- Rectified Flow Objective ---
        # 1. Sample a random time t from [0, 1]
        t = torch.rand(batch, device=device)
        t_reshaped = t.reshape(-1, 1, 1)

        # 2. Sample random noise z_0 from a standard Gaussian
        noise = torch.randn_like(image_embed)

        # 3. Form the training input z_t by linearly interpolating between noise and data
        z_t = t_reshaped * image_embed + (1 - t_reshaped) * noise

        # 4. The target is the constant velocity vector of the straight path
        target_velocity = image_embed - noise

        # Create conditioning dictionary for the network
        text_cond = dict(text_embed=text_embed)

        # 5. Predict the velocity using the PriorNetwork
        predicted_velocity = self.net(
            z_t,
            t, # Pass continuous time t
            text_cond_drop_prob=self.text_cond_drop_prob,
            **text_cond
        )

        # 6. The loss is the mean squared error between the predicted and target velocities
